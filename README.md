...
Copilot CLI runs **Claude Sonnet 4.6** by default — the same Anthropic model family that powers Claude Code. You can switch to Sonnet 5, Opus 5, Fable 5.1, Haiku 4.5, GPT-5.x/GPT-6 Astra, Gemini 3.8 Flash, or free models (GPT-5 mini, GPT-4.1) via `/model`. The mental model, commands, and externalization patterns map directly across agentic CLIs, which is why these patterns transfer.
